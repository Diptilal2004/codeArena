...
export default function PlaygroundRepl () {
    ...
    const Languages = {"cjs" : "javascript" , "md" : "markdown" , "html" : "html" , "json" : "json" , "css" :"css" , "tsx" : "javascript" , "ts" : "javascript" , undefined : null } 
    ...
    useEffect(() => {
      ...
      if(playgroundName && playgroundType && socket){
        ...
        socket.emit("providefile" , playgroundName , playgroundType ,  "App.tsx" ,  (content : string) => {
          // @ts-ignore 
          console.log("again , requesting for files");
    ...
    useEffect(() => {
      if(playgroundName){
        ...
        socket.emit("provide-preview-url" , playgroundName , (url : any) => {
            setPreviewUrl(url);
        })
      };
    } , [socket]);
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        if(activeTab[0] && activeTab[0].fileName){
          // @ts-ignore 
          console.log(Languages[activeTab[0].fileName.split('.')[activeTab[0].fileName.split('.').length - 1]] , ": language")
        }
      } , [activeTab])
                      ...
                      <ResizablePanelGroup  direction="vertical" className='min-w-[400px] min-h-[900px]'>
                            <ResizablePanel defaultSize={65} className='border border-[#2b2b2b] min-h-[300px] '>
                                <section className='flex flex-col h-full w-full bg-[#1e1e1e]' >  
                                  ...
                                  //@ts-ignore
                                  <Editor ref={editorRef} language={activeTab[0] && Languages[activeTab[0].fileName.split('.')[activeTab[0].fileName.split('.').length - 1]] ? Languages[activeTab[0].fileName.split('.')[activeTab[0].fileName.split('.').length - 1]] : "" } theme='vs-dark' value={activeTab[0]?.content} onChange={ (e) => {setActiveTab([{fileName : activeTab[0].fileName  , content : e}])}} />   
                                   : 
                                  <Loading />}
                                </section>
                            </ResizablePanel>
